...
class TestResource(JumpstarterTest):
    ...
    def test_setup_device(self, client, console):
        ...
        try:
            # wget https://github.com/mangelajo/jumpstarter-test-images/raw/refs/heads/main/raspbian-minimal-console.raw.xz
            # use the get-image devfile task action to get this file
            client.dutlink.storage.write_local_file("./raspbian-minimal-console.raw")
        except opendal.exceptions.NotFound:
            pytest.exit(
                "No image found, use the get-image action or download from https://github.com/mangelajo/jumpstarter-test-images/raw/refs/heads/main/raspbian-minimal-console.raw.xz"
            )
            return
        client.dutlink.storage.dut()
        client.dutlink.power.on()
    ...
    def test_power_on_hdmi(self, client, video):
        ...
        client.dutlink.power.on()
        time.sleep(1)
        video.assert_snapshot("tests/test_booting_empty_ok.jpeg")
        time.sleep(6)
        video.assert_snapshot("tests/test_booting_rainbow_ok.jpeg")
        time.sleep(4)
        video.assert_snapshot("tests/test_booting_raspberries_ok.jpeg")
        client.dutlink.power.off()
